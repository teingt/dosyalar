...
const TopBar = (props) => {
    ...
    useEffect(() => {
        document.addEventListener('click', menuClikTracker);
        return () => {
            document.removeEventListener('click', menuClikTracker)
        };
    }, [isLoggedIn]);

    const menuClikTracker = (event) => {
    ...
    const onLogoutSuccess = () => {
        ...
        let links = (
            ...
            if(isLoggedIn){
                ...
                links = (
                <ul className = "navbar-nav nl-auto" ref={menuArea}>                     
